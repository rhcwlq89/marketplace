> **시리즈**: 대용량 시스템 스터디 가이드
> - **이전**: [Phase 2 - 캐싱 전략](./02_CACHING_GUIDE.md)
> - **현재**: Phase 3 - 이벤트 드리븐 아키텍처
> - **다음**: [Phase 4 - Resilience 패턴](./04_RESILIENCE_GUIDE.md)
> - **전체 목차**: [STUDY_GUIDE.md](./STUDY_GUIDE.md)

---

# 이벤트 드리븐 아키텍처 & 메시지 큐 스터디 가이드

이 문서는 마켓플레이스 프로젝트를 통해 이벤트 드리븐 아키텍처와 메시지 큐의 개념을 학습합니다.
...
```kotlin
// Consumer offset을 되돌려 재처리
@KafkaListener(topics = ["marketplace.order.created"])
fun reprocessFromBeginning(
